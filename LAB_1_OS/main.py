...
from Creator import make_binary
...
# for i in range(5):
#     tmp = threading.Thread(target=func, name=f"thread-N0{i}" )
#     list_of_threads.append(tmp)
#     tmp.start()


def main():
    name_of_binary = input()
    number_of_people = int(input())
    name_of_binary = make_binary(name_of_binary, number_of_people)
    with open(name_of_binary, "rb") as f:
        print(f)
    new_name_of_binary = input()
    name_of_txt = input()
    per_hour = int(input())
    txt = make_reports(new_name_of_binary, name_of_txt, per_hour)
    with open(txt, "r") as file_69:
        print(file_69)
